...
#' @param studentsonly return only users enrolled with student role
#'
...
mdl_userlist <- function(dbp, prefix, courseid, returngroups = FALSE, studentsonly = TRUE){
  # checkout pool
  conn <- pool::poolCheckout(dbp)
  on.exit(pool::poolReturn(conn))
  query <- mdl_userlist_query(prefix, courseid, studentsonly = studentsonly)
...
mdl_userlist_query <- function(prefix, courseid, studentsonly = TRUE) {
  if(studentsonly){
